package modules.kitchen.service
{
	 ...
	 * @author cloud
	 */
	public class Command2DService extends BaseRL2Service
	{
		[Inject]
...
//			_loader.LoadByCode((evt.data as L3DMaterialInformations).code);
		...
		{
			if(dispatcher.hasEventListener(CommandTypeDict.CMD_CREATE_KICHENPART))
				dispatcher.dispatchEvent(new DatasEvent(CommandTypeDict.CMD_CREATE_KICHENPART,_loader.mesh));
		}
		override protected function addListener():void
		{
			_loader.addEventListener(Event.COMPLETE,onLoadComplete);
			context.addEventListener(CommandTypeDict.CMD_GET_MATERIAL_INFO,onLoadMesh);
		}
		override protected function removeListener():void
		{
			_loader.removeEventListener(Event.COMPLETE,onLoadComplete);
			context.removeEventListener(CommandTypeDict.CMD_GET_MATERIAL_INFO,onLoadMesh);
		}

package modules.kitchen.view
{
	...
	public class CabinetViewSetMediator extends BaseMediator
	{
		...
		private function onMouseMove(evt:MouseEvent):void
		{
			...
			scene.calculateMouseRay(CVector3DUtil.RAY_3D,evt.localX,evt.localY);
			Geometry3DUtil.instance.intersectPlaneByRay(CVector3DUtil.RAY_3D,CVector3DUtil.ZERO,Vector3D.Z_AXIS,_intersectPos);
			...
			for each(var vo:CObject3DVO in vos)
			{
				for (var i:int=0; i<cabinetSet.meshes.length; i++)
				{
					...
					if(mesh && mesh.UniqueID==vo.uniqueID)
					{
						if(vo.isLife)
						{
							mesh.rotationZ=MathUtil.instance.toRadians(vo.rotation);
...
//					mesh.Dispose(false);
		...
		private function onCreateKitchenFurniture(evt:DatasEvent):void
		{
			...
			(ModelManager.instance.getModel(mesh.catalog) as ICFurnitureModel).createFurnitureVo(mesh.UniqueID,MathUtil.instance.toDegrees(mesh.rotationZ),mesh.catalog,mesh.Length,mesh.Width,mesh.Height);
		}
		...
		{
			var vos:Vector.<CObject3DVO>=cabinetModel.createTableBoards();
			if(vos)
			{
		...
		private var decorationModule:CWaistDecorationModuleImp;
		private function createShelter():void
		{
			var vos:Vector.<CObject3DVO>=cabinetModel.createShelterVos();
			if(vos)
			{
...
//			taskVo.url="LIBRARY\\JD20170411201325387427\\YX-3R30472BP.L3D";
			...
			decorationModule=new CWaistDecorationModuleImp();
			decorationModule.addDecorationTask(taskVo);
		}
		...
		private function onInitDoubleList(evt:DatasEvent):void
		{
			cabinetModel.rootList=KitchenGlobalModel.instance.initKitchenListByWalls(Object3DDict.OBJECT3D_CABINET,wallModel.walls);
			hangingCabinetModel.rootList=KitchenGlobalModel.instance.initKitchenListByWalls(Object3DDict.OBJECT3D_CABINET,wallModel.walls);
		}
